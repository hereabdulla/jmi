...
def get_total_working_hours_vcp_vcr(from_date ,to_date,plant):
	if plant == '':
		...
		attendance = frappe.db.sql("""select docstatus,name,employee,status,shift,in_time,out_time,attendance_date,branch,designation from `tabAttendance` where attendance_date between '%s' and '%s' and branch = '%s' """%(from_date,to_date,plant),as_dict=True)	
	for att in attendance:
		if (att.branch == 'VCP' or att.branch == 'VCR') and att.docstatus != 2:
...
def mark_attendance_from_checkin(checkin,employee,time,device,log_type,plant):
	...
	if log_type == "IN":
		...
		if device in ['BRM9213960395','BRM9222360378','BRM9232760084','BRM9222360379','BRM9222361360','BRM9232260887']:
			max_out = datetime.strptime('10:00:00', '%H:%M:%S').time()
			if att_time < max_out:
				yesterday = add_days(att_date,-1)
				checkins = frappe.db.sql("select name,time from `tabEmployee Checkin` where employee = '%s' and device_id in ('BRM9213960395','BRM9222360378','BRM9232760084','BRM9222360379','BRM9222361360','BRM9232260887') and date(time) = '%s' and time(time) < '%s' order by time  ASC"%(employee,att_date,max_out),as_dict=True)            
				...
				checkins = frappe.db.sql("select name,time from `tabEmployee Checkin` where employee ='%s' and device_id in ('BRM9213960395','BRM9222360378','BRM9232760084','BRM9222361360','BRM9232260887','BRM9222360379') and date(time) = '%s' order by time ASC"%(employee,att_date),as_dict=True)
...
# def get_api_log():
# 	serial_number_list=["BRM9222360384","BRM9213960395","BRM9203461488","BRM9222360378","BRM9193660282","BRM9232760084","BRM9211160652","BRM9222361360","BRM9222361258","BRM9215260842","BRM9222361257","BRM9232260887"]
