...
class AttendanceRegularize(Document):
	def validate(self):
		if self.attendance_marked:
			self.corrected_total_working_hours = time_diff_in_hours(self.corrected_out_time , self.corrected_in_time)
			if self.corrected_total_working_hours > 8:
				corrected_o_t = self.corrected_total_working_hours - 8
				if self.branch == 'JMI Plant 1':
					if float(corrected_o_t) >= 7:
						self.corrected_overtime_hours = 8
					elif float(corrected_o_t)  >= 4:
						self.corrected_overtime_hours = 4.5
					elif float(corrected_o_t) >= 2:
						self.corrected_overtime_hours = 2.50
					elif  float(corrected_o_t) < 2:
						self.corrected_overtime_hours = 0
				if not (self.branch == 'JMI Plant 1'):
					if float(corrected_o_t) >= 7:
						self.corrected_overtime_hours = 8
					elif float(corrected_o_t)  >= 3:
						self.corrected_overtime_hours = 3.50
					elif  float(corrected_o_t) < 2:
						self.corrected_overtime_hours = 0
				if not (self.corrected_total_working_hours > 8):
					self.corrected_overtime_hours = 0
				# if self.branch == 'JMI Plant 1':
				# 	if float(corrected_o_t) >= 7:
				# 		self.corrected_overtime_hours = 7.50
				# 	elif float(corrected_o_t)  >= 4:
				# 		self.corrected_overtime_hours = 4.50
				# 	elif float(corrected_o_t) >= 2:
				# 		self.corrected_overtime_hours = 2.50
				# 	elif  float(corrected_o_t) < 2:
				# 		self.corrected_overtime_hours = 0
				# if self.branch in ['JMI Plant 2', 'JMI Plant 3', 'VCT', 'Seyoon']:
				# 	if float(corrected_o_t) >= 7:
				# 		self.corrected_overtime_hours = 7.50
				# 	elif float(corrected_o_t)  >= 4:
				# 		self.corrected_overtime_hours = 3.50
				...
				# 	elif  float(corrected_o_t) < 2:
				# 		self.corrected_overtime_hours = 0
				# if self.branch in ['VCP', 'VCR']:
				# 	if float(corrected_o_t) >= 7:
				# 		self.corrected_overtime_hours = 8
				# 	elif float(corrected_o_t)  >= 4:
				# 		self.corrected_overtime_hours = 4
				# 	elif float(corrected_o_t) >= 2:
				# 		self.corrected_overtime_hours = 2.50
				# 	elif  float(corrected_o_t) < 2:
				# 		self.corrected_overtime_hours = 0
	...
	def on_submit(self):
		...
		if not hh:
			...
			if att:
				...
				attendance.total_working_hours = self.corrected_total_working_hours
				attendance.over_time_hours = self.corrected_overtime_hours
	...
	def on_cancel(self):
		...
		if att:
			...
			if att_reg == self.name:
				...
				frappe.db.sql(""" update `tabAttendance` set status = 'Absent' where name ='%s' """%(att))
	
	def validate_total_wh(self):
		datalist = []
		data = {}
		work_hour = time_diff_in_hours(self.corrected_out_time,self.corrected_in_time)
		str_in=str(self.corrected_in_time).split('.')[0]
		str_out=str(self.corrected_out_time).split('.')[0]
		str_in_time=datetime.strptime(str_in,'%Y-%m-%d %H:%M:%S')
		str_out_time=datetime.strptime(str_out,'%Y-%m-%d %H:%M:%S')
...
def attendance_marked(emp, att_date):
	datalist = []
	if frappe.db.exists('Attendance', {'employee': emp, 'attendance_date': att_date}):
		att = frappe.get_doc('Attendance', {'employee': emp, 'attendance_date': att_date})
		in_time = att.in_time if att.in_time else ''
		out_time = att.out_time if att.out_time else ''
		total_working_hours = att.total_working_hours if att.total_working_hours else ''
		overtime_hours = att.over_time_hours if att.over_time_hours else ''
		status_ = att.status if att.status else ''
		att_id = att.name if att.name else ''
